...
/**
 ...
 @author Sean
 */
public class AppointmentsReportScreenController implements Initializable {
    
    @FXML private final ObservableList<Month> monthsList = FXCollections.observableArrayList();

    public static final String typeColumnMapKey = "type";
    public static final String totalColumnMapKey = "amount";
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        appointmentTypeTableColumn.setCellValueFactory(new MapValueFactory(typeColumnMapKey));
        totalAppointmentsTableColumn.setCellValueFactory(new MapValueFactory(totalColumnMapKey));
    ...
    public ObservableList<HashMap> initializeMapData(List<Appointment> totalAppointmentsList) {
        ...
        totalAppointmentsList.stream().map(appointment -> {
            ...
            String selectedType = appointment.getType();
            int total = 0;
            
            // find how many appointments have the same type in the list, iterate total
            total = totalAppointmentsList.stream().filter(apt -> (apt.getType().equals(selectedType))).map(a -> 1).reduce(total, Integer::sum);
            // puts the type and total of the current appointment in the current map, which is then added to the reportList
            row.put(typeColumnMapKey, selectedType);
            row.put(totalColumnMapKey, String.valueOf(total));
            return row;
